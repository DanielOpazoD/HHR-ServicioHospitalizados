...
import { CategoryCounts, CudyrCategory } from '@/services/cudyr/cudyrSummary';
    ...
    <div className="flex items-center justify-between mb-3 pb-2 border-b border-slate-100">
      ...
      <div
        className={clsx(
          ...
          total > 0 ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-400'
        )}
      >
        {total}
      </div>
...
export const CudyrSummaryTable: React.FC<CudyrSummaryTableProps> = ({
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4 print:hidden">
      <MiniTable title="Camas UTI" icon="🏥" counts={counts.uti} total={utiTotal} />
      <MiniTable title="Camas Medias" icon="🛏️" counts={counts.media} total={mediaTotal} />
    </div>
  );
};

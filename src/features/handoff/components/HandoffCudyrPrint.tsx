        ...
        <div className="flex justify-between items-center mb-1">
          ...
          <div className="flex gap-3 text-[10px] font-bold bg-slate-50 px-2 py-1 rounded-md border border-slate-200 print:bg-white print:border-slate-300">
            <div className="flex flex-col items-center leading-none">
              <span className="text-slate-400 text-[8px] uppercase tracking-tighter mb-0.5">
                Ocupadas
              </span>
          ...
          <table className="w-full text-left text-xs border-collapse border border-slate-300 table-fixed min-w-[720px] print:table-auto print:min-w-0 print:text-[7px]">
            <thead>
              ...
              <tr className="text-center">
                ...
                <th className="border border-slate-300 p-1 w-32 bg-slate-50 align-middle print:w-auto">
                  NOMBRE
                </th>
                <th className="border border-slate-300 p-1 w-20 bg-slate-50 align-middle print:w-auto">
                  RUT
                </th>

                <VerticalHeader
                  text="Cuidados Cambio Ropa"
                  colorClass="bg-blue-50 print:bg-white"
                />
                <VerticalHeader
                  text="Cuidados de Movilización"
                  colorClass="bg-blue-50 print:bg-white"
                />
                <VerticalHeader
                  text="Cuidados de Alimentación"
                  colorClass="bg-blue-50 print:bg-white"
                />
                <VerticalHeader
                  text="Cuidados de Eliminación"
                  colorClass="bg-blue-50 print:bg-white"
                />
                <VerticalHeader
                  text="Apoyo Psicosocial y Emocional"
                  colorClass="bg-blue-50 print:bg-white"
                />
                <VerticalHeader text="Vigilancia" colorClass="bg-blue-50 print:bg-white" />

                <VerticalHeader
                  text="Medicición Signos Vitales"
                  colorClass="bg-red-50 print:bg-white"
                />
                <VerticalHeader text="Balance Hìdrico" colorClass="bg-red-50 print:bg-white" />
                <VerticalHeader
                  text="Cuidados de Oxigenoterapia"
                  colorClass="bg-red-50 print:bg-white"
                />
                <VerticalHeader
                  text="Cuidados diarios de Vía Aérea"
                  colorClass="bg-red-50 print:bg-white"
                />
                <VerticalHeader
                  text="Intervenciones Profesionales"
                  colorClass="bg-red-50 print:bg-white"
                />
                <VerticalHeader
                  text="Cuidados de la Piel y Curaciones"
                  colorClass="bg-red-50 print:bg-white"
                />
                <VerticalHeader
                  text="Administración Tto Farmacológico"
                  colorClass="bg-red-50 print:bg-white"
                />
                <VerticalHeader
                  text="Presencia Elem. Invasivos"
                  colorClass="bg-red-50 print:bg-white"
                />
              ...
              {visibleBeds.map(bed => {
                ...
                if (!patient?.patientName) {
                  return (
                    ...
                    >
                      ...
                      <td
                        colSpan={16}
                        className="border-r border-slate-300 p-1 text-center text-slate-400 italic text-[10px] print:text-[8px]"
                      >
                        Cama disponible
                      </td>
                ...
                return (
                  ...
                  >
                    ...
                    {/* Name Column */}
                    <td
                      className="border-r border-slate-300 p-1 font-medium text-slate-700 text-[10px] truncate max-w-[120px]"
                    ...
                    {/* RUT Column */}
                    <td className="border-r border-slate-300 p-1 text-center text-[10px] whitespace-nowrap">
                      {patient.rut || '-'}
                    </td>

                    {/* Dependency scores with borders */}
                    <td className="border-r border-slate-300 p-1 text-center">
                      {renderScore(cudyr.changeClothes)}
                    </td>
                    <td className="border-r border-slate-300 p-1 text-center">
                      {renderScore(cudyr.mobilization)}
                    </td>
                    <td className="border-r border-slate-300 p-1 text-center">
                      {renderScore(cudyr.feeding)}
                    </td>
                    <td className="border-r border-slate-300 p-1 text-center">
                      {renderScore(cudyr.elimination)}
                    </td>
                    <td className="border-r border-slate-300 p-1 text-center">
                      {renderScore(cudyr.psychosocial)}
                    </td>
                    <td className="border-r border-slate-300 p-1 text-center">
                      {renderScore(cudyr.surveillance)}
                    </td>

                    {/* Risk scores with borders */}
                    <td className="border-r border-slate-300 p-1 text-center">
                      {renderScore(cudyr.vitalSigns)}
                    </td>
                    <td className="border-r border-slate-300 p-1 text-center">
                      {renderScore(cudyr.fluidBalance)}
                    </td>
                    <td className="border-r border-slate-300 p-1 text-center">
                      {renderScore(cudyr.oxygenTherapy)}
                    </td>
                    <td className="border-r border-slate-300 p-1 text-center">
                      {renderScore(cudyr.airway)}
                    </td>
                    <td className="border-r border-slate-300 p-1 text-center">
                      {renderScore(cudyr.proInterventions)}
                    </td>
                    <td className="border-r border-slate-300 p-1 text-center">
                      {renderScore(cudyr.skinCare)}
                    </td>
                    <td className="border-r border-slate-300 p-1 text-center">
                      {renderScore(cudyr.pharmacology)}
                    </td>
                    <td className="border-r border-slate-300 p-1 text-center">
                      {renderScore(cudyr.invasiveElements)}
                    </td>

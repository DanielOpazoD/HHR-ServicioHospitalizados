import type { ClinicalAttachmentFileKind } from '@/features/clinical-documents/domain/clinicalAttachmentTypes';
...
export const resolveClinicalAttachmentFilePolicy = (
  ...
  if (!fileKind) {
    return {
      ...
      message: 'Solo se permiten imagenes, PDF y documentos DOCX como adjuntos clinicos.',
    };
  }
  ...
  if (file.size > CLINICAL_ATTACHMENT_DOCUMENT_MAX_BYTES) {
    return {
      ...
      fileKind,
      code: 'file_too_large',
      message: 'El archivo supera el limite permitido para adjuntos clinicos.',
    };
  }

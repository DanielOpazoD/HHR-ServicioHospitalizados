...
export const ConsolidationManager: React.FC = () => {
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      auditConsolidationLogger.error('Preview failed', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      auditConsolidationLogger.error('Execution failed', error);
    } finally {
      setLoading(false);
      setProgress('');
    }
        ...
        {shellState.showEmptyState && (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-slate-500 max-w-sm mx-auto">
              Analiza la base de datos para encontrar registros redundantes generados por ráfagas de
              edición o errores de sincronización.
            </p>
          </div>
        )}
              ...
              <div className="border border-slate-100 rounded-xl overflow-hidden">
                ...
                {preview.duplicateGroups.length > 10 && (
                  <div className="p-2 bg-slate-50 text-[10px] text-center text-slate-400 italic">
                    Mostrando los primeros 10 grupos de {preview.duplicateGroups.length}{' '}
                    encontrados...
                  </div>
                )}
              </div>

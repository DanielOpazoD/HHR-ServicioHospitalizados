        ...
        {Object.entries(userActivity)
          ...
          .map(([userId, userLogs]) => {
            ...
            const sortedLogs = [...userLogs].sort(
              (a, b) =>
                parseAuditTimestamp(a.timestamp).getTime() -
                parseAuditTimestamp(b.timestamp).getTime()
            );
                ...
                <div className="relative pl-6">
                        ...
                        className={clsx(
                          ...
                          log.action === 'USER_LOGIN'
                            ...
                            : log.action === 'USER_LOGOUT'
                              ? 'bg-rose-500'
                              : getActionCriticality(log.action) === 'critical'
                                ? 'bg-amber-500'
                                : 'bg-slate-300'
                        )}
                      />
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] text-slate-400 font-mono w-12">
                            {parseAuditTimestamp(log.timestamp).toLocaleTimeString('es-CL', {
                              hour: '2-digit',
                              minute: '2-digit',
                            })}
                          </span>
                  ...
                  {sortedLogs.length > 10 && (
                    <p className="text-[10px] text-slate-400 pl-4">
                      +{sortedLogs.length - 10} más acciones...
                    </p>
                  )}
                </div>
              </div>

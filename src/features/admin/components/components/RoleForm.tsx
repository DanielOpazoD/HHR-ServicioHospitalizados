  ...
  return (
    <div
      className={`bg-white p-5 rounded-[1.75rem] shadow-xl transition-all duration-500 border ${
        editingEmail ? 'border-indigo-500 shadow-indigo-100/50' : 'border-slate-100'
      }`}
    >
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        <div>
          ...
          <input
            ...
            autoFocus
            className={`w-full px-4 py-3 border rounded-2xl focus:ring-4 focus:ring-indigo-50/50 outline-none transition-all font-bold text-slate-700 placeholder:text-slate-200 text-sm ${
              editingEmail
                ? 'border-indigo-400 bg-indigo-50/10'
                : 'border-slate-100 focus:border-indigo-500 bg-white'
            }`}
          />
          {email && !isValidEmail && (
        ...
        <button
          ...
          className={`w-full px-4 py-3.5 rounded-2xl text-white font-black uppercase tracking-widest text-[11px] transition-all shadow-lg active:scale-[0.96] flex items-center justify-center gap-2.5 ${
        ...
        >
          {processing ? (
            ...
            <>
              {editingEmail ? <CheckCircle2 size={16} /> : <UserPlus size={16} />}

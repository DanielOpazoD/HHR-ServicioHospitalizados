    ...
    <div className="flex items-center gap-4 p-3 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 hover:border-slate-300 transition-all group">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <h3 className="font-medium text-slate-800 text-sm truncate">{file.title}</h3>
          {isDay ? (
        ...
        <div className="flex items-center gap-3 text-xs text-slate-500 mt-0.5">
          ...
          <span className="flex items-center gap-1">
            <Clock size={12} />
            {formatBackupClockTime(file.createdAt)}
          </span>
        </div>
      </div>
      ...
      <div className="hidden sm:flex flex-col items-end text-right">
        <span className="text-xs font-medium text-slate-600">
          {formatBackupDisplayDateVerbose(file.date)}
        </span>
        <span className="text-xs text-slate-400">{file.metadata.patientCount} pacientes</span>
      </div>

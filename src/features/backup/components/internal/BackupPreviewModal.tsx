...
export const BackupPreviewModal: React.FC<BackupPreviewModalProps> = ({
  ...
  const typeConfig = file ? getBackupTypePresentation(file.type) : null;
  const shiftConfig = file?.shiftType ? getBackupShiftPresentation(file.shiftType) : null;
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-4 text-[10px] font-black uppercase tracking-widest text-slate-400">
            ...
            {shiftConfig && (
              <span className="bg-slate-100 px-2 py-1 rounded-md">{shiftConfig.label}</span>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-slate-50 border border-slate-100 rounded-xl p-3">
              ...
              <p className="text-sm font-bold text-emerald-600">
                {formatBackupTimestamp(file.createdAt)}
              </p>
            </div>
          ...
          <div className="flex items-center gap-2 p-3 bg-blue-50/50 rounded-xl border border-blue-100/50">
            ...
            <div>
              ...
              <p className="text-[11px] font-bold text-slate-700">
                {file.createdBy.name}{' '}
                <span className="font-normal text-slate-400">&lt;{file.createdBy.email}&gt;</span>
              </p>
            </div>
          </div>
        </div>

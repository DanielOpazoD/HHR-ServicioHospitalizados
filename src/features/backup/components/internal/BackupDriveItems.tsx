...
export const FolderCard: React.FC<FolderCardProps> = ({ name, onClick, itemCount }) => (
  ...
  >
    ...
    <div className="flex-1 min-w-0">
      ...
      {itemCount !== undefined && <p className="text-xs text-slate-400">{itemCount} elementos</p>}
    </div>
    <ChevronRight
      size={16}
      className="text-slate-300 group-hover:text-teal-400 transition-colors"
    />
  </button>
);
...
export const FileCard: React.FC<FileCardProps> = ({
  ...
  <div
    className={`flex items-center gap-3 ${compact ? 'p-2' : 'p-3'} bg-white border border-slate-200 rounded-xl hover:bg-slate-50 hover:border-teal-300 transition-all group`}
  >
    ...
    <div className="flex-1 min-w-0">
      <h4 className={`font-medium text-slate-800 ${compact ? 'text-xs' : 'text-sm'} truncate`}>
        {name}
      </h4>
        ...
        {!hideShift && shift && (
          <span
            className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold uppercase ${
              shift === 'day' ? 'bg-amber-100 text-amber-700' : 'bg-indigo-100 text-indigo-700'
            }`}
          >
    ...
    {path.map((item, index) => (
      <React.Fragment key={index}>
        ...
        <button
          ...
          className={`hover:text-teal-600 font-medium transition-colors ${
            index === path.length - 1 ? 'text-teal-600' : ''
          }`}
        >
          {item}
        </button>

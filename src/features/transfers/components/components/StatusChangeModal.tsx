    ...
    >
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-center gap-4 py-4">
          <div
            className={`px-4 py-2 rounded-xl font-medium ${currentConfig.bgColor} ${currentConfig.color}`}
          >
            {currentConfig.label}
          </div>
          ...
          {nextConfig ? (
            <div
              className={`px-4 py-2 rounded-xl font-medium ${nextConfig.bgColor} ${nextConfig.color} ring-2 ring-offset-2 ring-blue-500`}
            >
              {nextConfig.label}
            </div>
          ) : (
          ...
          <div>
            ...
            <div className="max-h-40 overflow-y-auto space-y-2">
              {transfer.statusHistory.map((change, index) => (
                <div
                  key={index}
                  className="flex items-start gap-2 text-sm bg-slate-50 p-2.5 rounded-xl"
                >
                  ...
                  <span className="text-slate-400 text-xs whitespace-nowrap">
                    {formatTransferVerboseDateTime(change.timestamp)}
                  </span>
                </div>
              ))}
            </div>

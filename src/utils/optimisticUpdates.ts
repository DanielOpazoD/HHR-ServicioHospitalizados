...
export async function performOptimisticUpdate<T>(
  ...
  const { currentState, optimisticState, updateFn, onOptimisticUpdate, onSuccess, onError } =
    options;
...
export function useOptimisticState<T>(options: UseOptimisticStateOptions<T>) {
  ...
  const updateOptimistically = useCallback(
    async (updater: T | ((current: T) => T)): Promise<boolean> => {
      // Store previous state for rollback
      previousStateRef.current = state;

      // Compute new state
      const newState =
        typeof updater === 'function' ? (updater as (current: T) => T)(state) : updater;

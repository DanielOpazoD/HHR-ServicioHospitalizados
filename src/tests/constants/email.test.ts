...
import {
  buildCensusEmailSubject,
  buildCensusEmailBody,
...
describe('Email Constants', () => {
  ...
  describe('buildCensusEmailBody', () => {
    ...
    it('should include encryption PIN when provided', () => {
      const body = buildCensusEmailBody('2025-12-28', undefined, '1212');
      expect(body).toContain('Clave Excel: 1212');
    });
    ...
    it('should have correct order: body, PIN, farewell, separator, signature', () => {
      const body = buildCensusEmailBody('2025-12-28', 'Test Nurse', '1212');

      const bodyIndex = body.indexOf('Junto con saludar');
      const pinIndex = body.indexOf('Clave Excel: 1212');

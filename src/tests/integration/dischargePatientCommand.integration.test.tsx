/**
 * End-to-end integration test for the discharge-patient command pipeline.
 *
 *   useDischargePatient (hook)
 *      ↓ derives actor from useAuth()
 *   executeDischargePatientCommand (application)
 *      ↓ validation + isAnonymousActor guard
 *      ↓ port.persistDischarge
 *   defaultDailyRecordDischargePatientPort (services/daily-record)
 *      ↓ buildDischargePatch
 *   updatePartial (services/repositories) — mocked at the boundary
 *
 * Mirrors the admit-patient integration test so the pilot can be evolved
 * with the same confidence the admit flow has.
 */
...
const baseHookInput = () => ({
  ...
  rut: '22.222.222-2',
  dischargeStatus: 'Vivo' as const,
  dischargeDate: '2026-05-03',
  recordDate: '2026-05-03',
  preservedLocation: 'Sala 5',
});

describe('dischargePatientCommand integration (hook → port → repository)', () => {
  beforeEach(() => {
  ...
  it('persists the cleared bed under beds.<bedId> and emits PATIENT_DISCHARGED with the auth email', async () => {
  ...
  it('blocks the discharge when no user is authenticated, never touching repository or audit', async () => {

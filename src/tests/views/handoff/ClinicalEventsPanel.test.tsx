...
describe('ClinicalEventsPanel', () => {
  ...
  it('shows the add form when clicking the add button', () => {
    ...
    expect(screen.getByPlaceholderText(/Nota \(opcional\)/i)).toHaveClass('min-w-0');
  ...
  it('calls onAdd with correct data', () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/Nombre del evento/i), {
      target: { value: 'Nuevo Evento' },
    });
    fireEvent.change(screen.getByPlaceholderText(/Nota \(opcional\)/i), {

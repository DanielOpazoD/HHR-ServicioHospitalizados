...
import { doc, setDoc, getDoc, getDocs, deleteDoc, query } from 'firebase/firestore';
import { auth } from '@/firebaseConfig';

describe('backupService', () => {
  type AuthMutable = { currentUser: { uid: string; email: string; displayName: string } | null };
  type FirestoreDocResult = Awaited<ReturnType<typeof getDoc>>;
  type FirestoreDocsResult = Awaited<ReturnType<typeof getDocs>>;
  ...
  beforeEach(() => {
    vi.resetAllMocks();
    // Mock auth user
    (auth as unknown as AuthMutable).currentUser = {
      uid: 'tester',
      email: 'test@example.com',
      displayName: 'Test User',
    };
    // Mock doc return
    vi.mocked(doc).mockReturnValue({ id: 'mock-id' } as unknown as ReturnType<typeof doc>);
  });
  ...
  describe('listBackupFiles', () => {
    it('should fetch and format list of backups', async () => {
      ...
      vi.mocked(getDocs).mockResolvedValue(mockSnap as unknown as FirestoreDocsResult);

...
describe('baseStorageService', () => {
  ...
  describe('Factory Functions', () => {
    ...
    it('createListYears returns [] for expected storage miss errors', async () => {
      const listYears = BaseStorage.createListYears('test-root');
      const { listAll } = await import('firebase/storage');
      vi.mocked(listAll).mockRejectedValueOnce({ code: 'storage/unauthorized' });
    ...
    it('createListFilesInMonth skips expected miss errors per file', async () => {
      const config: BaseStorage.ListFilesConfig<BaseStorage.BaseStoredFile> = {
        storageRoot: 'test-root',
        parseFilePath: vi.fn().mockReturnValue({ date: '2025-01-01' }),
        mapToFile: vi.fn(),
      ...
      vi.mocked(listAll).mockResolvedValueOnce({
        prefixes: [],
        items: [{ name: 'no-access.json', fullPath: 'path' }],
      } as unknown as ListResult);
      vi.mocked(getMetadata).mockRejectedValueOnce({ code: 'storage/unauthorized' });

...
const resolveInvalidTimeMessage = (time: string): string => {
  const timeResult = TimeSchema.safeParse(time);
  return timeResult.success
    ? ''
    : timeResult.error.issues[0]?.message || 'Formato de hora inválido. Use HH:mm (24h).';
};
...
export const validateDischargeExecutionInput = ({
  ...
  const invalidTimeMessage = resolveInvalidTimeMessage(time);

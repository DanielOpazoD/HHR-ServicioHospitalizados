...
export const TestAgent: React.FC<TestAgentProps> = ({ isRunning, onComplete, currentRecord }) => {
  ...
  const addLog = useCallback(
    (message: string, status: 'pending' | 'success' | 'error' = 'pending') => {
  ...
  const runDiagnostics = useCallback(async () => {
    ...
    if (currentRecord) {
      ...
      const manualCount = Object.values(currentRecord.beds).filter(
        (b: PatientData) => b.patientName || b.isBlocked
      ).length;
      ...
      if (Math.abs(manualCount - calculatedTotal) <= 1) {
        // Tolerance for logic differences
      ...
      <div className="bg-slate-900 text-green-400 font-mono rounded-lg shadow-2xl w-full max-w-lg border border-green-500/30 overflow-hidden">
        <div className="bg-slate-800 p-3 border-b border-slate-700 flex justify-between items-center">
          <span className="flex items-center gap-2 font-bold">
            <Bot size={18} /> Agente de Diagnóstico v1.0
          </span>

...
export const DemoModePanel: React.FC<DemoModePanelProps> = ({ isOpen, onClose }) => {
  const { isActive, period, startDate, isGenerating, activateDemo, deactivateDemo, setGenerating } =
    useDemoMode();
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (selectedPeriod === 'day') {
        ...
        setGenerationStatus(
          `Generando mes completo (${new Date(year, month - 1).toLocaleDateString('es-CL', { month: 'long', year: 'numeric' })})...`
        );
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="p-4 bg-amber-50 rounded-xl border border-amber-200">
            <p className="text-sm text-amber-800">
              <strong>Nota:</strong> Los datos demo se almacenan por separado. Al desactivar el modo
              demo, sus datos reales estarán intactos.
            </p>
          </div>
        </div>

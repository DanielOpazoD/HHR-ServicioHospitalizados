...
import { uiSettingsLogger } from '@/services/storage/storageLoggers';
...
export const loadUISettings = (): UISettings => {
  try {
    ...
    uiSettingsLogger.error('Error loading UI settings', error);
  }
...
export const saveUISettings = (settings: UISettings): void => {
  try {
    ...
    uiSettingsLogger.error('Error saving UI settings', error);
  }
};

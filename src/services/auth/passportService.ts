...
const verifySignature = async (passport: OfflinePassport): Promise<boolean> => {
  ...
  if (version === `v${PASSPORT_VERSION + 1}`) {
    ...
    return (
      ...
      passport.signature === `v${PASSPORT_VERSION + 1}-${expectedHash}-${timestamp}`
    );
  }
...
export const validatePassport = async (
  ...
  // E2E test-only bypass: never enabled in production/runtime environments.
  ...
  if (expiresAt < new Date()) {
    return {
      valid: false,
      error: 'El pasaporte ha expirado. Regenere uno nuevo con conexión a internet.',
    };
  }
 ...
 */
export const verifyPassportCredentials = async (
  passport: OfflinePassport,
  email: string,
  password: string
): Promise<boolean> => {
...
export const downloadPassport = async (user: AuthUser, targetRole?: string): Promise<boolean> => {
  ...
  if (typeof window.showSaveFilePicker === 'function') {
    try {
      ...
      console.warn(
        '[Passport] showSaveFilePicker failed, falling back to traditional method:',
        error
      );
    }
  }
...
export const parsePassportFile = async (file: File): Promise<OfflinePassport | null> => {
  try {
    ...
    // Parse JSON
    const passport = safeJsonParse<OfflinePassport | null>(jsonContent, null);
    if (!passport) {
      return null;
    }
...
export const getStoredPassport = async (): Promise<OfflinePassport | null> => {
  try {
    ...
    const legacy =
      typeof window !== 'undefined' ? localStorage.getItem('hhr_offline_passport') : null;
    if (legacy) {
      try {
        const parsed = safeJsonParse<OfflinePassport | null>(legacy, null);

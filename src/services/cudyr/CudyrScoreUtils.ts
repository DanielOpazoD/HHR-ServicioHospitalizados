 ...
 * Returns empty finalCat if patient has no CUDYR scores (not categorized)
 */
export const getCategorization = (cudyr: CudyrScore | undefined) => {
  ...
  // Only show category if patient has at least one score
  const finalCat = isCategorized ? `${riskCat}${depCat}` : '';
  const badgeColor = isCategorized ? getCategoryColor(riskCat) : 'bg-slate-200';

...
interface CensusMasterWorkbookOptions {
...
export const buildCensusMasterWorkbook = async (
  ...
  const buildSheetDescriptors = (options?: CensusMasterWorkbookOptions) => {
    ...
    if (providedDescriptors.length === 0) {
      return sortedRecords.map((record, index) => ({
        ...
        descriptor: {
          ...
          sortOrder: index,
        } satisfies CensusWorkbookSheetDescriptor,
      }));
    }
    ...
    return [...providedDescriptors]
      ...
      .map(descriptor => {
        ...
        if (!record) {
          throw new Error(
            `No se encontró registro para la hoja configurada: ${descriptor.recordDate} (index ${String(descriptor.recordLookupIndex)})`
          );
        }
  ...
  buildSheetDescriptors(options).forEach(({ record, descriptor }) => {
...
function sanitizeSheetName(name: string): string {
  ...
  const fallback = normalized || 'Hoja';
  return fallback.slice(0, EXCEL_SHEET_NAME_MAX_LENGTH);
}
...
function reserveUniqueSheetName(baseName: string, usedNames: Set<string>): string {
  ...
  let suffix = 2;
  while (suffix < 1000) {
    const rawCandidate = `${sanitizedBase} (${suffix})`;
    const candidate = rawCandidate.slice(0, EXCEL_SHEET_NAME_MAX_LENGTH);
  ...
  const timestampName = `Hoja-${Date.now()}`.slice(0, EXCEL_SHEET_NAME_MAX_LENGTH);

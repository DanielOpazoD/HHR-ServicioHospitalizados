...
export function expandSynonyms(query: string): string[] {
  ...
  // Collect ALL matching expansions (not just the first)
  const allExpansions = new Set<string>();

  // Check each token individually for abbreviation matches
  const tokens = normalized.split(/\s+/);
  for (const token of tokens) {
    if (DIAGNOSIS_SYNONYMS[token]) {
      for (const exp of DIAGNOSIS_SYNONYMS[token]) {
        allExpansions.add(exp);
      }
    }
  }

  // Also check for compound term matches (only for multi-word queries or long terms)
  // Skip this for very short queries (≤3 chars) to avoid false positives
  // like "iam" matching "siameses" via key.includes(normalized)

...
run_firestore_emulator_exec \
  "npm run build && npm run test:rules && npm run test:emulator:sync && npm run test:emulator:ui && npm run test:e2e:critical && npm run test:e2e:flow-performance:built && npm run check:flow-performance-budget"

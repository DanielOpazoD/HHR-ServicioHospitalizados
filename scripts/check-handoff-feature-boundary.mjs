...
const FEATURE_ROOT = path.normalize(path.join(ROOT, 'src', 'features', 'handoff'));
const PUBLIC_MODULES = new Set([
  '@/features/handoff',
  '@/features/handoff/index',
  '@/features/handoff/public',
]);
...
if (violations.length > 0) {
  console.error('\nHandoff feature boundary violations:');
...
console.log('Handoff feature boundary checks passed.');

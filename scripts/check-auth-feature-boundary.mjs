...
const AUTH_FEATURE_ROOT = path.normalize(path.join(ROOT, 'src', 'features', 'auth'));
const AUTH_PUBLIC_MODULES = new Set(['@/features/auth', '@/features/auth/index', '@/features/auth/public']);
...
for (const absolutePath of files) {
  if (absolutePath.startsWith(AUTH_FEATURE_ROOT)) {
    continue;
  }
  ...
  while ((match = IMPORT_EXPORT_REGEX.exec(source)) !== null) {
    ...
    if (!importPath || AUTH_PUBLIC_MODULES.has(importPath)) continue;
    const resolved = resolveImport(absolutePath, importPath);
    if (resolved && resolved.startsWith(AUTH_FEATURE_ROOT)) {
...
if (violations.length > 0) {
  console.error('\nAuth feature boundary violations:');
...
console.log('Auth feature boundary checks passed.');

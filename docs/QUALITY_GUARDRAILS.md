...
- `ci:release-gate` valida `check:release-evidence`: primero exige `check:report-freshness:strict` y luego bloquea si los reportes ejecutivos fueron generados desde un checkout con cambios locales significativos o si falta la evidencia dedicada del smoke visual clínico (`reports/e2e/clinical-visual-release-report.json`). El comando diario `check:report-freshness` es advisory para evitar que cada merge requiera una rama de reportes nueva.
